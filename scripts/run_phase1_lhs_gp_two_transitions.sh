...
cleanup(){
 if [[ "$RUN_COMPLETED" != "1" ]]; then python3 -m app.lab.cli experiment finish --id "$EXPERIMENT_ID" --state paused --message "interrupted; safe to resume" >/dev/null 2>&1 || true; fi
 python3 - <<'PY'
...
python3 -m app.lab.cli experiment init --id "$EXPERIMENT_ID" --mode "$MODE" --lhs "$LHS" --gp "$GP" >/dev/null
python3 -m app.lab.cli environment inspect >/dev/null
python3 -m app.lab.cli migration report >/dev/null
printf '[progress] readable report: artifacts/experiments/%s/live_progress.md\n' "$EXPERIMENT_ID"
for workload in tpcc twitter; do
 if [[ "$MODE" == "formal" ]]; then python3 -m app.lab.cli workload prepare "$workload" --reload; fi
 python3 -m app.lab.cli tune reference "$workload" --experiment-id "$EXPERIMENT_ID" --lhs "$LHS" --gp "$GP" --duration "$DURATION" --terminals "$TERMINALS" "${ISOLATION[@]}"
 python3 -m app.lab.cli reference select "$workload" --experiment-id "$EXPERIMENT_ID" --duration "$DURATION" --terminals "$TERMINALS" --repeats "$REPEATS" "${ISOLATION[@]}"
done
python3 -m app.lab.cli transition run tpcc twitter --experiment-id "$EXPERIMENT_ID" --duration "$DURATION" --terminals "$TERMINALS" "${STRICT[@]}"
python3 -m app.lab.cli transition run twitter tpcc --experiment-id "$EXPERIMENT_ID" --duration "$DURATION" --terminals "$TERMINALS" "${STRICT[@]}"
python3 - <<PY
from pathlib import Path
import json
...
PY
python3 -m app.lab.cli experiment finish --id "$EXPERIMENT_ID" --state completed --message "all phases completed" >/dev/null
RUN_COMPLETED=1
